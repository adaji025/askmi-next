...
import { HelpCircle, MapPin, Building2, User, Users, Sparkles } from "lucide-react"
    ...
    <div className="p-4 sm:p-6 lg:px-8 bg-white space-y-12 rounded-lg border border-[#E2E8F0]! shadow-none!">
      ...
      <section className="space-y-6">
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="campaign-name" className="text-sm font-semibold text-muted-foreground">
              Campaign name
            </Label>
            <Input id="campaign-name" placeholder="Name your campaign" className="h-12 bg-white" />
          </div>
          <div className="space-y-2">
          ...
          >
            <div
              className={cn(
                ...
                surveyType === "existing" ? "bg-primary" : "bg-muted-foreground/30",
              )}
            />
            <h3 className="font-bold mb-1">Use existing survey</h3>
            <p className="text-sm text-muted-foreground">Choose from your existing surveys</p>
          </button>
          <button
            onClick={() => setSurveyType("new")}
            className={cn(
              "flex flex-col items-start p-6 rounded-xl border transition-all text-left group",
              surveyType === "new" ? "border-primary bg-primary/5" : "border-border bg-white hover:border-primary/50",
            )}
          >
            <div
              className={cn(
            ...
            <p className="text-sm text-muted-foreground">Build a new survey from scratch</p>
          </button>
        </div>
      </section>
      ...
      <section className="space-y-6">
        ...
        <div className="flex flex-wrap gap-3">
          <AudienceSelect icon={<MapPin className="w-4 h-4" />} label="Regions" />
          <AudienceSelect icon={<Building2 className="w-4 h-4" />} label="Cities" />
          <AudienceSelect icon={<User className="w-4 h-4" />} label="Ages" />
          <AudienceSelect icon={<Users className="w-4 h-4" />} label="Genders" />
          <AudienceSelect icon={<Sparkles className="w-4 h-4" />} label="Interests" />
        </div>
      </section>
      ...
      <section className="space-y-8">
        ...
        <div className="space-y-6">
          <div className="flex items-end justify-between">
            <Label className="text-sm font-semibold text-muted-foreground mb-2">Total votes needed</Label>
            <span className="text-2xl font-bold">{votes[0].toLocaleString()}</span>
          </div>
          <div className="space-y-4">
            <Slider
              value={votes}
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <Label className="text-sm font-semibold text-muted-foreground">Deviation range</Label>
      ...
      <div className="flex justify-end pt-4">
        <Button className="px-10 h-12 text-base font-semibold bg-primary hover:bg-primary/90 rounded-xl transition-all active:scale-95">
          Continue
        </Button>
      </div>
    </div>
...
function AudienceSelect({ icon, label }: { icon: React.ReactNode; label: string }) {
  return (
    <Select>
